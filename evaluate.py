...
def render_flat_rays(model, rays, chunks=8192):
    """
    分批渲染展平的光线束，每批渲染结果立即转回 CPU，返回到CPU上的浮点型RGB（范围[0,1]）
    解决一次性渲染整张图显存不足的问题
    Args:
    ...
    Returns:
        torch.Tensor: CPU上的RGB结果，形状为[num_rays, 3]
...
def evaluate_test_set(config):
    ...
    # 采样后的图像尺寸列表
    image_shapes = [image_shapes[i] for i in sampled_indices]
    print(f"[INFO] 测试图像数量: {len(image_shapes)}")
    ...
    for image_idx, dataset_img_idx in enumerate(tqdm(sampled_indices, desc="Evaluating")):
        ...
        pred_rgb = render_flat_rays(model, rays, chunks=config.chunks).reshape(h, w, 3)
